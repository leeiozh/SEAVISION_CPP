...
TEST(TEST_PICTURE, PICTURE_SIMPLE) {
    ...
    std::shared_ptr<Polygon::BaseVisualField> field = std::make_shared<Polygon::CircleVisualField>(
            Eigen::Vector2d(2. / deg, 1. / deg),
            Eigen::Vector2d(matrixParams.matr_x, matrixParams.matr_y), "/home/leeiozh/mmcp/polygon/Polygon/resources");

    Eigen::Vector3d scope_dir = {1, 0, 0};
    Eigen::Vector3d scope_pos = {6370e3, 1, 0};
    ScopeState scope_state = {scope_pos, scope_dir};
    Eigen::Vector3d sun_pos = {-1.496e11, 0, 0.};

    SatState sat = {1, 0.2, {6770e3, 0, 0}};
    ...
    auto aber_func_ptr = std::make_shared<AberrationFunction<SIZE_X, SIZE_Y>>(c_coeffs, s_coeffs);
    ...
    auto atm_noise = std::make_shared<MoffatAtmosphericNoise>(1e-1, 0.5);

    PSF<SIZE_X, SIZE_Y> psf(aber_func_ptr, delta_func_ptr, trans_func_ptr, atm_noise);
    SkyLight<SIZE_X, SIZE_Y> sky(1e-17);
    ...
    auto res = picture.calc_pictures(scope_state, sat_state, sun_pos, 1e-5);
