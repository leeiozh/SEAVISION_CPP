...
public:
     ...
     * @param atm_noise
     */
    inline PSF(const std::shared_ptr<AberrationFunction<SIZE_X, SIZE_Y>> &aber_func,
        ...
        comp_matrix = (fftw_complex *) fftw_malloc(SIZE_X * SIZE_Y * sizeof(fftw_complex));
...
//                std::cout << i << " " << j << std::endl;
                ...
                double aber = aber_func->calc_aberration_func(i, j);
                ...
                comp_matrix[SIZE_Y * i + j][0] = number.real();
                comp_matrix[SIZE_Y * i + j][1] = number.imag();

            }
        }

        fftw_complex *fft_matrix;
        fft_matrix = (fftw_complex *) fftw_malloc(SIZE_X * SIZE_Y * sizeof(fftw_complex));
        ...
        fftw_destroy_plan(p);
        fftw_cleanup();
        fftw_free(comp_matrix);

        // вычисление квадрата модуля прообраза
        for (int i = 0; i < SIZE_X; ++i) {
            for (int j = 0; j < SIZE_Y; ++j) {
                double real = fft_matrix[i * SIZE_Y + j][0];
                double imag = fft_matrix[i * SIZE_Y + j][1];
                matrix(i, j) = real * real + imag * imag;
            }
        }
        fftw_free(fft_matrix);
    };
