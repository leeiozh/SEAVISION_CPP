...
public:
     ...
     * @param index current index in main process
     * @param step step in input backscatter
     * @return index of zone with the most vertical wave crests
     */
    [[nodiscard]] Eigen::VectorXi calc_directions(const Eigen::MatrixXi &data, int index, double step);
     ...
     * @return length
     */
    Eigen::VectorXd calc_length(const Eigen::MatrixXi &data, double step);
    ...
    /**
     * @return significant wave height calculated by dispersion
     */
    [[nodiscard]] Eigen::VectorXd get_swh() const;

    /**
     * @return wave length calculated by 1D Fourier transform
     */
    [[nodiscard]] Eigen::VectorXd get_len() const;

    /**
     * @return wave directions calculated by dispersion
     */
